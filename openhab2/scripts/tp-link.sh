...
check(){
	toutput=`sendtoplug $ip $port "$payload_query"`
	echo $toutput
...
altcheck(){
	output=`status "$payload_query"`
        IFS=',' read -r -a array <<< "$output"
	for element in "${array[@]}"
        do
		if [[ $element == \"relay* ]] ;
		then
			IFS=':' read -r -a relay <<< "$element"
			if [[ ${relay[1]} == "1" ]] ;
			then
				echo ON
			else
...
check_binaries
checkargs
logger "tp-link $ip cmd=$cmd"
case "$cmd" in
	ON|100)
		sendtoplug $ip $port "$payload_on" > /dev/null
		logger "tp-link $ip turned on"
		echo ON
		;;
	OFF|0)
		sendtoplug $ip $port "$payload_off" > /dev/null
		logger "tp-link $ip turned off"
		echo OFF
		;;
	check)
		altcheck
